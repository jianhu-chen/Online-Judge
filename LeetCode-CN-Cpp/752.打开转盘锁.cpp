...
public:
    string plusOne(string s, int pos)
    {
        ...
        {
            s.replace(pos, 1, string(1, s[pos] + 1));
        }
        return s;
    }
    ...
    int openLock(vector<string> &deadends, string target)
    {
        ...
        while (!q.empty())
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                {
                    string plus_ret = plusOne(s, j);
                    string minus_ret = minusOne(s, j);
                    if (visited.find(plus_ret) == visited.end())
                    {
                        visited.insert(plus_ret);
                        q.push(plus_ret);
                    }
                    if (visited.find(minus_ret) == visited.end())
                    {
                        visited.insert(minus_ret);
                        q.push(minus_ret);
                    }
                }
            }
